import type { Breed } from "../../../types/breed";

export default {
  id: 358,
  details: {
    internal: "thai_bangkaew",
    public: ["Thai Bangkaew Dog"],
  },
  classification: {
    fci: {
      group: 5,
      section: 5,
      standardNumber: 358,
    },
  },
  podcast: [
    {
      number: 218,
      episode: "Reiherpfoten auf die Ohren - so war der ESC",
      sources: [
        {
          url: "https://open.spotify.com/episode/6gtrPiAGmCWuzhuLNjq9AN",
          type: "audio",
          provider: "spotify",
        },
        {
          url: "https://plus.rtl.de/podcast/tierisch-menschlich-der-podcast-mit-hundeprofi-martin-ruetter-und-katharina-adick-m5iuweomug8fv/218-reiherpfoten-auf-die-ohren-so-war-der-esc-n21vg9dgh2ixb",
          type: "audio",
          provider: "rtl",
        },
      ],
      meta: {
        internal: "portrait",
        public: "Rasseportrait",
        timecode: 3063,
        airDate: "2025-05-22",
        isGuessable: true,
        isGuessedCorrectly: false,
        guessedBy: "mr",
      },
    },
  ],
  furtherReading: [
    {
      name: "Wikipedia",
      url: "https://de.wikipedia.org/wiki/Thai_Bangkaew_Dog",
    },
    {
      name: "FCI",
      url: "https://www.fci.be/de/nomenclature/THAI-BANGKAEW-HUND-358.html",
    },
    {
      name: "VDH",
      url: "https://welpen.vdh.de/hunderassen/rasselexikon/ergebnis/thai-bangkaew-dog",
    },
  ],
} satisfies Breed;
